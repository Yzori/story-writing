...
// Crossroads) live in the Director's row / rituals. Players choose between two
// intents only — write what their character does, or speak (audit P1 #5). The
// Director narrates the world or answers the table's actions with consequence.
const PLAYER_TYPES = [
...
/**
 * The dock's writing surface — the old bottom composer, re-homed as an
 * IN-FLOW block (never position:fixed; the shell's grid owns the layout).
 * Same draft/speech hooks, same commit contract.
 */
    ...
    <div className="w-full">
      <div className="mb-2.5 flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-baseline gap-3">
          <p className="shrink-0 text-[10px] uppercase tracking-[0.18em] text-amber">{composerTitle}</p>
          <p className="hidden truncate text-[11.5px] text-text-tertiary sm:block">{composerHint}</p>
        </div>
          ...
          <div className="flex gap-1.5">
            {typeOptions.map((option) => (
              <button
                ...
                className={`min-h-8 shrink-0 rounded-full border px-3 text-[10px] font-bold uppercase tracking-[0.12em] transition-colors ${
      ...
      {!isGM && renderPreview[draftType] && (
        <div className="mb-2 px-1 font-serif text-[11px] italic text-text-ghost">
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
        <div className="space-y-2">
          <textarea
            ...
            className="min-h-[76px] w-full resize-none rounded-lg border border-border bg-elevated px-4 py-3 font-reading text-[16px] leading-relaxed text-paper outline-none transition-colors placeholder:text-text-ghost focus:border-amber/35"
          />
